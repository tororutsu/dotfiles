    ...
    while true; do
        ...
        IFS=$'\t' read -ra tags <<< "$(herbstclient tag_status "${MON_IDX}")"
        {
            for i in "${tags[@]}" ; do
		        ...
		        # echo "%{F#345F0C} 0%{F-}"
		         echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{B#282A2E}  ${i:1}  %{B-}%{A}"
                        ;;
                    ':')
                        # the tag is not empty
                        # TODO Add your formatting tags
		        echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{F#000000}%{B#A59D9D} ${i:1} %{F-}%{B-}%{A}"
                        ;;
                    '+')
                        # the tag is viewed on the specified MONITOR, but this monitor is not focused.
                        # TODO Add your formatting tags
		        echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{F#000000}%{B#B082F2}  ${i:1}  %{F-}%{B-}%{A}"
                        ;;
                    '#')
                        # the tag is viewed on the specified MONITOR and it is focused.
                        # TODO Add your formatting tags
		        echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{F#000000}%{B#65159f}  ${i:1}  %{F-}%{B-}%{A}"
                        ;;
                    '-')
		        ...
		        echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{F#000000}%{B#65159f}  ${i:1}  %{F-}%{B-}%{A}"
                    ...
                    '!')
                        ...
                        # TODO Add your formatting tags
		        echo "%{A1:herbstclient focus_monitor ${MON_IDX}; herbstclient use ${i:1}:}%{F#000000}%{B#FF0000} [${i:1}] %{F-}%{B-}%{A}"
                        ;;
                esac
